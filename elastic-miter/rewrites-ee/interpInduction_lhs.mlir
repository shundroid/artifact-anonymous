module {
  handshake.func @interpInduction_lhs(%short: !handshake.channel<i1>, %long: !handshake.channel<i1>) -> (!handshake.channel<i1>) attributes {argNames = ["short_in", "long_in"], resNames = ["B_out"]} {
    ...
    %b = ee__interpolator %short, %long_forked#0 {handshake.name = "interpolate"} : <i1>
    %c = ee__repeating_init %long_forked#1 {handshake.name = "ee__repeating_init", initToken = 1 : ui1} : <i1>
