...
#define GEN_PASS_DEF_HANDSHAKEEE
...
struct HandshakeEEPass
    : public dynamatic::experimental::EE::impl::HandshakeEEBase<
          HandshakeEEPass> {
  using HandshakeEEBase<HandshakeEEPass>::HandshakeEEBase;
...
static PasserOp performMuxPasserSwap(MuxOp muxOp) {
  ...
  // Add repeating inits
  EERepeatingInitOp ri1 =
      builder.create<EERepeatingInitOp>(muxOp->getLoc(), init.getOperand(), 1);
  inheritBB(init, ri1);
  init.getOperandMutable()[0].set(ri1.getResult());

  assertMaterialization(passerOp.getCtrl());
  EERepeatingInitOp ri2 =
      builder.create<EERepeatingInitOp>(muxOp->getLoc(), passerOp.getCtrl(), 1);
...
static EERepeatingInitOp moveRepeatingInitsUp(Value fedValue) {
  fedValue = getForkTop(fedValue);
  materializeValue(fedValue);

  Operation *uniqueUser = getUniqueUser(fedValue);
  if (auto ri = dyn_cast<EERepeatingInitOp>(uniqueUser)) {
...
static EEInterpolatorOp introduceIdentInterpolator(Value value) {
  assertMaterialization(value);

  OpBuilder builder(value.getContext());
  builder.setInsertionPointAfterValue(value);

  EEInterpolatorOp interpolatorOp =
      builder.create<EEInterpolatorOp>(value.getLoc(), value, value);
...
static bool isInterpolatorInductionEligible(EEInterpolatorOp interpOp) {
  ...
  if (!defOp || !isa<EERepeatingInitOp>(defOp)) {
    llvm::errs() << "rhs of AndI not defined by repeating init\n";
    return false;
  }

  auto ri = cast<EERepeatingInitOp>(defOp);
  ...
  while (true) {
    ...
    if (auto ri = dyn_cast<EERepeatingInitOp>(
...
static EEInterpolatorOp
performInterpolatorInduction(EEInterpolatorOp interpOp) {
  ...
  EEInterpolatorOp newInterpOp = builder.create<EEInterpolatorOp>(
...
// static Value moveRepeatingInitDown(EERepeatingInitOp ri) {
...
//   for (auto res : fork->getResults()) {
//     EERepeatingInitOp newRI =
//         builder.create<EERepeatingInitOp>(ri.getLoc(), res, 1);
//     inheritBB(ri, newRI);
...
// static void moveTopRIAndPasser(EEInterpolatorOp interpolator) {
//   auto topRI = cast<EERepeatingInitOp>(
...
//     llvm_unreachable("EE algorithm failed");
//   }
// }

// /// Returns if the circuit is eligible for the introduction of the resolver.
// static bool
// isEligibleForResolverIntroduction(EEInterpolatorOp interpolator)
// {
//   // Ensure the structure
//   Operation *shortOperandDefiningOp =
//       interpolator.getShortOperand().getDefiningOp();
//   if (!isa<EERepeatingInitOp>(shortOperandDefiningOp)) {
//     llvm::errs() << "No repeating init\n";
//     return false;
//   }
//   auto riOp = cast<EERepeatingInitOp>(shortOperandDefiningOp);
...
// static EEResolverOp
// introduceSpecResolver(EEInterpolatorOp interpolator) {
//   auto riOp = cast<EERepeatingInitOp>(
//       (interpolator.getShortOperand().getDefiningOp()));
...
//   auto resolverOp = builder.create<EEResolverOp>(
...
void HandshakeEEPass::runDynamaticPass() {
  ...
  SmallVector<EERepeatingInitOp> repeatingInits(n);
  ...
  if (!disableInitialMotion) {
    ...
    for (auto passer : funcOp.getOps<PasserOp>()) {
      if (llvm::find(loopBBs, getLogicBB(passer)) == loopBBs.end())
        continue;
      passer->setAttr("EE_frontier", builder.getBoolAttr(false));
      ...
      do {
        ...
        for (auto passer : frontiers) {
          if (isPasserSimplifiable(passer)) {
            ...
            newFrontier->setAttr("EE_frontier", builder.getBoolAttr(false));
  ...
  for (unsigned i = 0; i < n; i++) {
    ...
    for (auto muxOp :
      ...
      passerOp->setAttr("EE_frontier", builder.getBoolAttr(false));
    ...
    if (i == 0) {
      newRI->setAttr("EE_top_ri", builder.getBoolAttr(true));
    }
  ...
  if (n >= 2) {
    ...
    for (unsigned i = 1; i < n; i++) {
      ...
      for (auto bottomPasser : bottomPassers) {
        ...
        tmpAnd->setAttr("EE_tmp_and", builder.getBoolAttr(true));
  ...
  for (auto passer : ctrlDefiningPassers) {
    ...
    if (exitEagerEval) {
      ...
      exitPasser->setAttr("EE_frontier", builder.getBoolAttr(false));
  ...
  if (resolver && n >= 2) {
    std::optional<EEInterpolatorOp> interpolator = std::nullopt;
    for (Operation *user :
         iterateOverPossiblyIndirectUsers(repeatingInits[0].getResult())) {
