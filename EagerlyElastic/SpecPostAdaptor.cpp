...
#define GEN_PASS_DEF_specPOSTADAPTOR
...
struct SpecPostAdaptorPass
    : public dynamatic::experimental::EE::impl::SpecPostAdaptorBase<
          SpecPostAdaptorPass> {
  using SpecPostAdaptorBase<SpecPostAdaptorPass>::SpecPostAdaptorBase;
  void runDynamaticPass() override;
};

void SpecPostAdaptorPass::runDynamaticPass() {
  ...
  for (auto branch :
    ...
    if (getLogicBB(branch) != *specBB)
      continue;
    if (branch->hasAttr("spec_cond_br"))
      continue;
    ...
    if (branch->hasAttr("spec_adaptor_inner_loop")) {
      trueResultPasser->setAttr("spec_adaptor_inner_loop",
                                branch->getAttr("spec_adaptor_inner_loop"));
      falseResultPasser->setAttr("spec_adaptor_inner_loop",
                                 branch->getAttr("spec_adaptor_inner_loop"));
    } else {
