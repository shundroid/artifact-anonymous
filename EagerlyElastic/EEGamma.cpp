...
struct EEGammaPass
    : public dynamatic::experimental::EE::impl::EEGammaBase<EEGammaPass> {
  using EEGammaBase<EEGammaPass>::EEGammaBase;
  void runDynamaticPass() override;
};

void EEGammaPass::runDynamaticPass() {
  ...
  if (stepsUntil >= 1) {
    ...
    if (stepsUntil >= 2) {
      ...
      if (oneSided) {
        ...
        auto ri =
            builder.create<EERepeatingInitOp>(builder.getUnknownLoc(), cond, 1);
  ...
  for (auto mux : llvm::make_early_inc_range(funcOp.getOps<MuxOp>())) {
    if (mux->hasAttr("EE_gsa_mux")) {
      ...
      inheritBB(mux, sink);
      sink->setAttr("EE_gsa_mux_nonpri",
                    builder.getIntegerAttr(builder.getIntegerType(32), gsaId));
      // sink->setAttr("EE_gsa_side",
